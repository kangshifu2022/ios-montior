...
- (BOOL)authenticateByPassword:(NSString *)password {
    ...
    NSArray *methods = [self supportedAuthenticationMethods];
    if (methods != nil && ![methods containsObject:@"password"]) {
        return NO;
    }
...
- (BOOL)authenticateByKeyboardInteractiveUsingBlock:(NSString *(^)(NSString *request))authenticationBlock {
    NSArray *methods = [self supportedAuthenticationMethods];
    if (methods != nil && ![methods containsObject:@"keyboard-interactive"]) {
        return NO;
    }
